...
def construct_label(tem_y, new_length, label_length, class_num):
    """
    For every lane change/cut in section, construct the corresponding Y label series
    :param tem_y:
    :param new_length:
    :param label_length:
    :param class_num:
    ...
    for index, label in enumerate(tem_y):
        if label != 0:
            ...
            # Prevent label exceeds the range
...
def construct_feature_lane_change(sub_path, columns_name, window_size, new_y_length, label_length, min_size_scenarios,
                                  class_num):
    """
...
def construct_feature_lane_change_with_lane(ego_file, dynamic_file, static_file, columns_name, window_size, new_y_length, label_length,
                             min_size_scenarios, class_num, object_slots_num, obj_dim, keep):

    feature_ls = np.zeros((ego_file.shape[0], 1))
    # Assign the lane change left and right flag
    for i in range(ego_file.shape[0]):
        if int(ego_file['lane_change_left'][i]) == 1:
            if float(ego_file['speed'][i]) > 60:
                feature_ls[i] = 1
            continue
        if int(ego_file['lane_change_right'][i]) == 1:
            if float(ego_file['speed'][i]) > 60:
                feature_ls[i] = 2

    index_ls = [idx for idx, item in enumerate(feature_ls) if item != 0]
    if len(index_ls) == 0:
        return -1, -1, False
    ...
    for idx, item in enumerate(feature_ls):
        if item != 0:
            ...
            if idx + r >= ego_file.shape[0] - 1 or idx + r - window_size < 0:
                continue
            temp_x = construct_cut_in_X(idx + r - window_size, idx + r, ego_file, dynamic, static, object_slots_num,
                                        window_size, columns_name, obj_dim, keep)
            # Select the part from whole cut_in_ls where cut in happen
            temp_y = feature_ls[idx + r - window_size:idx + r]
            temp_y = construct_label(temp_y, new_y_length, label_length, class_num)
            x.append(temp_x)
            y.append(temp_y)
...
def normalization_cut_in(data, columns_name, object_slots_num, obj_dim):
    """
    Use sklearn to normalize the data
    :param data: Data with shape (time_steps, feature_dims)
    :return: Normalized data with it's per feature mean and standard deviation
    ...
    # Append objects from all channels to "obj_dim" columns and calculate its mean and scale
    new_other_column = np.empty((0, obj_dim), float)
    for i in range(object_slots_num):
        temp = other_column[:, i * obj_dim:i * obj_dim + obj_dim]
    ...
    for i in range(object_slots_num):
        data[:, ego_column_num + obj_dim * i:ego_column_num + obj_dim * i + obj_dim] = (data[:,
                                                                      ego_column_num + obj_dim * i:ego_column_num + obj_dim * i + obj_dim] - scaler.mean_) / scaler.scale_
...
def cut_in_list(dynamic_df, ego_df):
    ...
    for i in range(dynamic_df.shape[0]):
        ...
        if (dynamic_df['cut_in_left'][i] == 1) and (dynamic_df['pos_x'][i] < 60):
            ...
            if ego_df['speed'][int(frame_num)] > 60:
                y_[frame_num] = 1
            continue
...
def construct_cut_in_X(start_frame, end_frame, ego_file, dynamic, static, object_slots_num, window_size, columns_name,
    ...
    temp_x = np.zeros((window_size, len(columns_name) + object_slots_num * obj_dim))
    ...
    i = 0
    for obj in dynamic:
        # Choose only first "object_slots_num"  object
        ...
        # The involved surrounding vehicle should show up in the selection
        if initial_obj_frame < end_frame and start_frame < end_obj_frame:
            ...
            if initial_obj_frame <= start_frame and end_obj_frame < end_frame and obj['pos_x'][
                start_frame - initial_obj_frame] < 80 and abs(obj['pos_y'][start_frame - initial_obj_frame]) < 15:
                if obj_dim > 2:
                    temp_x[:end_obj_frame - start_frame, len(columns_name) + i * obj_dim] = obj['pos_x'][
                                                                                            start_frame - initial_obj_frame:]
                    temp_x[:end_obj_frame - start_frame, len(columns_name) + 1 + i * obj_dim] = obj['pos_y'][
                                                                                                start_frame - initial_obj_frame:]
                    temp_x[:end_obj_frame - start_frame, len(columns_name) + 2 + i * obj_dim] = obj['speed_x'][
                                                                                                start_frame - initial_obj_frame:]
                    temp_x[:end_obj_frame - start_frame, len(columns_name) + 3 + i * obj_dim] = obj['speed_y'][
                                                                                                start_frame - initial_obj_frame:]
                else:
                    temp_x[:end_obj_frame - start_frame, len(columns_name) + i * obj_dim] = obj['pos_x'][
                                                                                            start_frame - initial_obj_frame:]
                    temp_x[:end_obj_frame - start_frame, len(columns_name) + 1 + i * obj_dim] = obj['pos_y'][
                                                                                                start_frame - initial_obj_frame:]
                    if keep:
                        temp_x[end_obj_frame - start_frame:, len(columns_name) + i * obj_dim] = obj['pos_x'][-1]
                        temp_x[end_obj_frame - start_frame:, len(columns_name) + 1 + i * obj_dim] = obj['pos_y'][-1]
                i += 1
                continue
            if initial_obj_frame > start_frame and end_obj_frame < end_frame and obj['pos_x'][0] < 80 and abs(
                    obj['pos_y'][0]) < 15:
                if obj_dim > 2:
                    temp_x[initial_obj_frame - start_frame:end_obj_frame - start_frame,
                    len(columns_name) + i * obj_dim] = obj['pos_x'][:]
                    temp_x[initial_obj_frame - start_frame:end_obj_frame - start_frame,
                    len(columns_name) + 1 + i * obj_dim] = obj['pos_y'][:]
                    temp_x[initial_obj_frame - start_frame:end_obj_frame - start_frame,
                    len(columns_name) + 2 + i * obj_dim] = obj['speed_x'][:]
                    temp_x[initial_obj_frame - start_frame:end_obj_frame - start_frame,
                    len(columns_name) + 3 + i * obj_dim] = obj['speed_y'][:]
                else:
                    temp_x[initial_obj_frame - start_frame:end_obj_frame - start_frame,
                    len(columns_name) + i * obj_dim] = obj['pos_x'][:]
                    temp_x[initial_obj_frame - start_frame:end_obj_frame - start_frame,
                    len(columns_name) + 1 + i * obj_dim] = obj['pos_y'][:]
                    if keep:
                        temp_x[:initial_obj_frame - start_frame, len(columns_name) + i * obj_dim] = obj['pos_x'][0]
                        temp_x[:initial_obj_frame - start_frame, len(columns_name) + 1 + i * obj_dim] = obj['pos_y'][0]
                        temp_x[end_obj_frame - start_frame:, len(columns_name) + i * obj_dim] = obj['pos_x'][-1]
                        temp_x[end_obj_frame - start_frame:, len(columns_name) + 1 + i * obj_dim] = obj['pos_y'][-1]
                i += 1
                continue
            if initial_obj_frame > start_frame and end_obj_frame >= end_frame and obj['pos_x'][0] < 80 and abs(
                    obj['pos_y'][0]) < 15:
                if obj_dim > 2:
                    temp_x[initial_obj_frame - start_frame:, len(columns_name) + i * obj_dim] = obj['pos_x'][
                                                                                                :end_frame - initial_obj_frame]
                    temp_x[initial_obj_frame - start_frame:, len(columns_name) + 1 + i * obj_dim] = obj['pos_y'][
                                                                                                    :end_frame - initial_obj_frame]
                    temp_x[initial_obj_frame - start_frame:, len(columns_name) + 2 + i * obj_dim] = obj['speed_x'][
                                                                                                    :end_frame - initial_obj_frame]
                    temp_x[initial_obj_frame - start_frame:, len(columns_name) + 3 + i * obj_dim] = obj['speed_y'][
                                                                                                    :end_frame - initial_obj_frame]
                else:
                    temp_x[initial_obj_frame - start_frame:, len(columns_name) + i * obj_dim] = obj['pos_x'][
                                                                                                :end_frame - initial_obj_frame]
            ...
            if initial_obj_frame < start_frame and end_obj_frame > end_frame and obj['pos_x'][
                    start_frame - initial_obj_frame] < 80 and abs(obj['pos_y'][start_frame - initial_obj_frame]) < 15:
                if obj_dim > 2:
                    temp_x[:, len(columns_name) + i * obj_dim] = obj['pos_x'][
                                                                 start_frame - initial_obj_frame:end_frame - initial_obj_frame]
                    temp_x[:, len(columns_name) + 1 + i * obj_dim] = obj['pos_y'][
                                                                     start_frame - initial_obj_frame:end_frame - initial_obj_frame]
                    temp_x[:, len(columns_name) + 2 + i * obj_dim] = obj['speed_x'][
                                                                     start_frame - initial_obj_frame:end_frame - initial_obj_frame]
                    temp_x[:, len(columns_name) + 3 + i * obj_dim] = obj['speed_y'][
                                                                     start_frame - initial_obj_frame:end_frame - initial_obj_frame]
                else:
                    temp_x[:, len(columns_name) + i * obj_dim] = obj['pos_x'][
                                                                 start_frame - initial_obj_frame:end_frame - initial_obj_frame]
...
def construct_feature(ego_file, dynamic_file, static_file, columns_name, window_size, new_y_length, label_length,
                             min_size_scenarios, class_num, object_slots_num, obj_dim, keep):
    """
    Construct the input and reference label for NN from every file group
    :param ego_file: ego_file from process_main
    ...
    # Find out where lane change and cut in happen. 1 = left cut in; 2 = right cut in; 3 = left lane change; 4 = right lane change"
    feature_ls = cut_in_list(dynamic_file, ego_file)
    ...
    for idx, item in enumerate(feature_ls):
        if item != 0:
            ...
            # Select the part from whole cut_in_ls where cut in happen
            temp_y = feature_ls[idx + r - window_size:idx + r]
            temp_y = construct_label_all(temp_y, new_y_length, label_length, class_num)
            x.append(temp_x)
            y.append(temp_y)
...
def construct_label_all(tem_y, new_length, label_length, class_num):
        ...
        if label != 0:
            ...
            if new_index + label_length > new_length - 1:
                ...
                if label == 3:
                    ...
                    new_y[new_index:new_index + label_length, 3] = 1
                    new_y[new_index:new_index + label_length, 0] = 0
                else:
                    new_y[new_index:new_index + label_length, 4] = 1
                    new_y[new_index:new_index + label_length, 0] = 0
    return new_y


def construct_feature_different_finish_time(ego_file, dynamic_file, static_file, columns_name, window_size, new_y_length, label_length,
                             min_size_scenarios, class_num, object_slots_num, obj_dim, keep, time_2_shift):
    """
    Construct the input and reference label for NN from every file group
    :param ego_file: ego_file from process_main
    :param dynamic_file: dynamic_file from process_main
    ...
    # Shift the label for time shifting
    feature_ls = np.roll(feature_ls, time_2_shift)
    if time_2_shift > 0:
        feature_ls[:time_2_shift] = 0
    else:
        feature_ls[time_2_shift:] = 0
