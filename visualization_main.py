...
def read_files(file_name):
    ...
    # Path 2
    # if os.path.exists(
    #         r'..\ma_haoyu\processed_data\csv\{}_dynamic.csv'.format(file_name)) and os.path.exists(
    #     r'..\ma_haoyu\processed_data\csv\{}_static.csv'.format(file_name)) and os.path.exists(
    #     r'..\ma_haoyu\processed_data\csv\{}_ego.csv'.format(file_name)):
    #     with open(r'..\ma_haoyu\processed_data\csv\{}_dynamic.csv'.format(file_name)) as tmp_dynamic:
    #         dynamic_csv = pd.read_csv(tmp_dynamic)
    #         print('Dynamic csv file found')
    #     with open(r'..\ma_haoyu\processed_data\csv\{}_static.csv'.format(file_name)) as tmp_static:
    #         static_csv = pd.read_csv(tmp_static)
    #         print('Static csv file found')
    #     with open(r'..\ma_haoyu\processed_data\csv\{}_ego.csv'.format(file_name)) as tmp_ego:
    #         ego_csv = pd.read_csv(tmp_ego)
    #         print('Ego csv file found')
    #     return ego_csv, dynamic_csv, static_csv
    else:
...
if __name__ == "__main__":
    ...
    ego, dynamic_raw, static_raw = read_files(r'0_cv')
    ...
    try:
        if dynamic is not None and static is not None and ego is not None:
            if play_video:
                # Path 1
                frame_count, cap = video(r'..\MDM data process\video\processed_video\20191029_130130_3015.avi')
                # Path 2
                # frame_count, cap = video(r'..\ma_haoyu\video\processed_video\20191029_130130_3015.avi')
                if frame_count > 0:
