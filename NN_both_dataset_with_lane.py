...
if __name__ == "__main__":
    ...
    # Why??????????????????????????????????????????????????????????????????????????????????? with lane??????????
    columns_name = ['speed', 'acc_x', 'acc_y', 'ego_line_left_distance_y', 'ego_line_right_distance_y']
    # How many classes
    class_num = 5
    ...
    cut_in_left_num = 0
    cut_in_right_num = 0
    lane_change_left_num = 0
    lane_change_right_num = 0
    ...
    for idx in range(len(file_ego_list)):
        ...
        x, y, exist_flag = construct_feature(ego_file, dynamic_file, static_file, columns_name, window_size,
                                                    new_y_length, label_length, min_size_scenarios, class_num,
                                                    object_slots_num, obj_dim, keep)
    ...
    np.save('{}\\X_all.npy'.format(path), X)
    np.save('{}\\Y_all.npy'.format(path), Y)
